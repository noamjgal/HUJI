# -*- coding: utf-8 -*-
"""
Created on Sun Jun  2 16:25:54 2024
...
def fetch_and_save_vehicle_data(date, output_filename="vehicles.shp"):
    ...
    for company, url in api_dict.items():
        ...
        try:
            ...
            print(f"Error parsing data from {company}: {e}")
        except Exception as e:
            print(f"Unexpected error processing data from {company}: {e}")
    ...
    vehicles_filename = f"{export_path}vehicles_data_{timestamp}.shp"
    route_filename = f"{export_path}routes_data_{timestamp}.shp"
    ...
    plot_filename = f"{export_path}map_plot_{timestamp}.png"
